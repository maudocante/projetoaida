...
MAINTAINER maudo cante <ummominhafilha@gmail.com>

COPY ./mkcert /mkcert 
RUN cd / && chmod +x /mkcert 
RUN cd / && ./mkcert -install && ./mkcert  cante.gw localhost 127.0.0.1 && cp ./cante.gw+2.pem /etc/ssl/certs/ssl-cert-snakeoil.pem && cp ./cante.gw+2-key.pem \
